...
import { BestiaireCategory, BestiaireItem, CategoryWithChildrenResponse } from '@models/bestiaire';
...
function mapCategory(category: CategoryRow): BestiaireCategory {
  return {
    id: category.id,
...
function mapBestiaireItem(item: BestiaireItemRow): BestiaireItem {
  return {
    id: item.id,
    title: item.title,
...
export const getCategoryWithChildren = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const response: CategoryWithChildrenResponse = {
      ...
      entries: category.items.map(mapBestiaireItem)
    };
...
export const createEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const item = await prisma.bestiaireItem.create({
      data: {
        title: title.trim(),
    ...
    await indexItem('bestiaire', item.id);

    res.status(201).json({ entry: mapBestiaireItem(item) });
  } catch (error) {
...
export const updateEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const item = await prisma.bestiaireItem.update({
      where: { id: entryId },
      data: updateData
    });
    ...
    res.json({ entry: mapBestiaireItem(item) });
  } catch (error) {
...
export const deleteEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const itemExists = await prisma.bestiaireItem.findUnique({
      where: { id: entryId }
    });
    ...
    await prisma.bestiaireItem.delete({
      where: { id: entryId }
    });
...
export const batchDeleteEntries = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.bestiaireItem.deleteMany({
      where: { id: { in: validIds } }
    });
