...
import { RoverCategory, RoverItem, CategoryWithChildrenResponse } from '@models/rover';
...
function mapCategory(category: CategoryRow): RoverCategory {
  return {
    id: category.id,
...
function mapItem(item: ItemRow): RoverItem {
  return {
    id: item.id,
    title: item.title,
...
export const deleteCategory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.roverCategory.delete({
      where: { id: categoryId }
    });
...
export const deleteEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const entryExists = await prisma.roverItem.findUnique({
      where: { id: entryId }
    });
...
export const batchDeleteCategories = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.roverCategory.deleteMany({
...
export const batchDeleteEntries = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.roverItem.deleteMany({
      where: { id: { in: validIds } }
    });
