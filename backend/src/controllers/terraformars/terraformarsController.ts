...
import { TerraformarsCategory, TerraformarsItem, CategoryWithChildrenResponse } from '@models/terraformars';
import { indexItem } from '@utils/elena/embeddings';

const UPLOAD_DIR = 'uploads/terraformars';
...
function mapCategory(category: CategoryRow): TerraformarsCategory {
  return {
    id: category.id,
...
function mapItem(item: ItemRow): TerraformarsItem {
  return {
    id: item.id,
    title: item.title,
...
export const getRootCategories = async (req: Request, res: Response): Promise<void> => {
  try {
    const categories = await prisma.terraformarsCategory.findMany({
      where: { parentId: null },
...
export const getCategoryWithChildren = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const category = await prisma.terraformarsCategory.findUnique({
      where: { id: categoryId },
      include: {
        children: {
          orderBy: { createdAt: 'asc' },
...
export const createCategory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (parentId !== null && parentId !== undefined) {
      const parentExists = await prisma.terraformarsCategory.findUnique({
    ...
    const category = await prisma.terraformarsCategory.create({
      data: {
        title: title.trim(),
...
export const createEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const categoryExists = await prisma.terraformarsCategory.findUnique({
    ...
    const item = await prisma.terraformarsItem.create({
      data: {
        title: title.trim(),
    ...
    await indexItem('terraformars', item.id);
...
export const updateCategory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const category = await prisma.terraformarsCategory.update({
      where: { id: categoryId },
...
export const updateEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const item = await prisma.terraformarsItem.update({
      where: { id: entryId },
      data: updateData
    });
...
export const deleteCategory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.terraformarsCategory.delete({
      where: { id: categoryId }
    });
...
export const deleteEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const entryExists = await prisma.terraformarsItem.findUnique({
      where: { id: entryId }
    });
    ...
    await prisma.terraformarsItem.delete({
      where: { id: entryId }
    });
...
export const batchDeleteCategories = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.terraformarsCategory.deleteMany({
...
export const batchDeleteEntries = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await prisma.terraformarsItem.deleteMany({
      where: { id: { in: validIds } }
    });
