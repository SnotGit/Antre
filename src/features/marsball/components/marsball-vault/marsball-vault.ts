...
import { MarsballCrudService, MarsballSection } from '../../services/marsball-crud.service';
...
export class MarsballVault {
  ...
  protected readonly opFeedback = inject(MarsballFeedbackService);
  private readonly itemEdit = inject(ItemEditService);
  ...
  protected readonly effectiveSection = computed<MarsballSection>(() =>
    this.itemSection() || (this.tree.activeSection() as MarsballSection) || 'marsball'
  );

  private readonly itemCatsResource = resource({
    params: () => ({ section: this.effectiveSection(), tick: this.tree.categories() }),
    loader: ({ params }) => this.crud.getAllCategories(params.section)
  });
  ...
  protected readonly showCategoryForm = computed(() => {
    ...
    if (this.tree.activeSection() === 'bestiaire' && this.tree.currentCategoryId() !== null) return false;
    return true;
  });
  ...
  private readonly _sectionEffect = effect(() => {
    const section = this.tree.activeSection();
    if (section) {
      this.crud.setSection(section);
    }
  });
  ...
  async createItemGo(): Promise<void> {
    ...
    const base = SECTION_ROUTES[this.effectiveSection()];
    await this.router.navigate([base, this.tree.slugify(target.title)], {
