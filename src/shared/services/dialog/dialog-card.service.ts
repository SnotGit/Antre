...
export interface DialogCardConfig {
  title: string;
  message: string;
  items?: string[];
...
export class DialogCardService {
  ...
  private dialogConfig = signal<DialogCardConfig | null>(null);
  ...
  showDialog(config: DialogCardConfig): Promise<boolean> {
  ...
  showMessage(config: DialogCardConfig): void {
    this.forceCloseDialog();
