...
export class VaultCategoriesService {
  ...
  private readonly _resource = resource({
    ...
    loader: ({ params }) => this.vaultGet.getAllCategories(params as SearchSection)
  });
  ...
  readonly currentCategoryId = computed<number | null>(() => {
    ...
    const match = cats.find(c => this.toSlug(c.title) === lastSlug);
  ...
  readonly breadcrumb = computed<BreadcrumbSegment[]>(() => {
    ...
    if (catId && typeof catId === 'number') {
      ...
      for (let i = 0; i < path.length; i++) {
        ...
        segments.push({
          label: cat.title,
          route: `${base}/${this.toSlug(cat.title)}`,
